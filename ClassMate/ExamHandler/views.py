...
def create_exam(req, class_id):
    ...
    if req.method == 'POST':
        ...
        return redirect(f'/set-question/{exam_id}/')
...
def set_question(req, exam_id):
    ...
    if req.method == 'POST':
        ...
        question_fullmarks = data.get('question_fullmarks')
        option_list = data.getlist('option')
        question_image = req.FILES.get('question_image')

        print(question)
        print(question_fullmarks)
        print(option_list)
